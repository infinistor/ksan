...
namespace PortalProviderInterface
{
    ...
    public interface IKsanUserProvider : IBaseProvider
	{
		...
		Task<ResponseData<ResponseAddKsanUser>> Add(RequestAddKsanUser request);

		/// <summary>Ksan 사용자를 수정한다.</summary>
		/// <param name="id">Ksan 사용자 식별자</param>
		/// <param name="request">Ksan 사용자 정보</param>
		/// <returns>Ksan 사용자 수정 결과</returns>
		Task<ResponseData> Update(string id, RequestAddKsanUser request);
		...
		Task<ResponseData<ResponseAddKsanUser>> GetUserByEmail(string email);

		/// <summary>Ksan 사용자 식별자로 특정 사용자를 가져온다.</summary>
		/// <param name="id">Ksan 사용자 식별자</param>
		/// <returns>해당 사용자 데이터</returns>
		Task<ResponseData<ResponseAddKsanUser>> GetUserById(string id);
		...
		Task<ResponseList<ResponseAddKsanUser>> GetUsers(int skip = 0, int countPerPage = 100,
