...
namespace PortalProvider.Providers.Accounts
{
	...
	public class S3UserProvider : BaseProvider<PortalModel>, IS3UserProvider
	{
		...
		public async Task<ResponseData<ResponseS3User>> Add(RequestS3User request)
		{
			ResponseData<ResponseS3User> Result = new ResponseData<ResponseS3User>();

			try
			{
				// 사용자명이 없는 경우
				if (request.Name.IsEmpty())
					return new ResponseData<ResponseS3User>(EnumResponseResult.Error, Resource.EC_COMMON__INVALID_INFORMATION, Resource.EM_COMMON_ACCOUNT_REQUIRE_NAME);
				...
				if (responseDuplicatedName.Result != EnumResponseResult.Success)
					return new ResponseData<ResponseS3User>(EnumResponseResult.Error, responseDuplicatedName.Code, responseDuplicatedName.Message);
				...
				if (!request.Email.IsEmpty())
				{
					...
					if (responseDuplicatedEmail.Result != EnumResponseResult.Success)
						return new ResponseData<ResponseS3User>(EnumResponseResult.Error, responseDuplicatedEmail.Code, responseDuplicatedEmail.Message);
					Email = request.Email;
				}
				...
				try
				{
					...
					//S3 사용자 등록
					await m_dbContext.S3Users.AddAsync(newData);
					await m_dbContext.SaveChangesWithConcurrencyResolutionAsync();
					Result.Result = EnumResponseResult.Success;
					Result.Data = (ResponseS3User)new ResponseS3User().CopyValueFrom(newData);
		...
		public async Task<ResponseData> Update(string id, RequestS3User request)
		{
			...
			try
			{
				...
				// 사용자 계정을 가져온다.
				S3User user = await m_dbContext.S3Users.Where(i => i.Id == guidId).FirstOrDefaultAsync();
				...
				//S3 사용자 변경 알림
				var responseS3User = new ResponseS3User();
				responseS3User.CopyValueFrom(user);
				SendMq(RabbitMqConfiguration.ExchangeName, "*.services.s3.user.updated", responseS3User);
			}
			catch (Exception ex)
			{
				NNException.Log(ex);
		...
		public async Task<ResponseData> Remove(string id)
		{
			...
			try
			{
				...
				// 사용자 계정을 가져온다.
				S3User user = await m_dbContext.S3Users.AsNoTracking().Where(i => i.Id == guidId).FirstOrDefaultAsync();
				...
				var responseS3User = new ResponseS3User();
				responseS3User.CopyValueFrom(user);
				SendMq(RabbitMqConfiguration.ExchangeName, "*.services.s3.user.removed", responseS3User);
			}
			catch (Exception ex)
			{
				NNException.Log(ex);
		...
		public async Task<ResponseList<ResponseS3User>> GetUsers(int skip = 0, int countPerPage = 100,
			List<string> orderFields = null, List<string> orderDirections = null,
			List<string> searchFields = null, string searchKeyword = "")
		{
			ResponseList<ResponseS3User> result = new ResponseList<ResponseS3User>();
			...
			try
			{
				...
				result.Data = await m_dbContext.S3Users.AsNoTracking().Where(i =>
					...
					.CreateListAsync<dynamic, ResponseS3User>(skip, countPerPage);
		...
		public async Task<ResponseData<ResponseS3User>> GetUserById(string id)
		{
			ResponseData<ResponseS3User> result = new ResponseData<ResponseS3User>();
			try
			{
				// 아이디가 유효하지 않은 경우
				if (id.IsEmpty() || !Guid.TryParse(id, out Guid guidId))
					return new ResponseData<ResponseS3User>(EnumResponseResult.Error, Resource.EC_COMMON__INVALID_REQUEST, Resource.EM_COMMON__INVALID_REQUEST);

				// 사용자 ID로 해당 사용자 정보를 반환한다.
				ResponseS3User user = await m_dbContext.S3Users.AsNoTracking().Where(i => i.Id == guidId).FirstOrDefaultAsync<S3User, ResponseS3User>();
		...
		/// <param name="id">사용자 식별자</param>
		/// <returns>사용자 정보 객체</returns>
		public async Task<ResponseData<ResponseS3User>> GetUserByName(string name)
		{
			ResponseData<ResponseS3User> result = new ResponseData<ResponseS3User>();
			try
			{
				...
				ResponseS3User user = await m_dbContext.S3Users.AsNoTracking().Where(i => i.Name == name).FirstOrDefaultAsync<S3User, ResponseS3User>();
		...
		/// <returns>사용자 정보 객체</returns>
		public async Task<ResponseData<ResponseS3User>> GetUserByEmail(string email)
		{
			ResponseData<ResponseS3User> result = new ResponseData<ResponseS3User>();
			try
			{
				// 사용자 식별자가 유효하지 않은 경우
				if (email.IsEmpty())
					return new ResponseData<ResponseS3User>(EnumResponseResult.Error, Resource.EC_COMMON__INVALID_INFORMATION, Resource.EM_COMMON_ACCOUNT_REQUIRE_EMAIL);
				// 사용자 이메일로 해당 사용자 정보를 반환한다.
				ResponseS3User user = await m_dbContext.S3Users.AsNoTracking().Where(i => i.Email == email).FirstOrDefaultAsync<S3User, ResponseS3User>();
