...
public class DataPutBucketReplication extends S3DataRequest {
    private String contentMD5;
	private String bucketObjectLockToken;
	private String expectedBucketOwner;
	private String replicationXml;

    public DataPutBucketReplication(S3Parameter s3Parameter) throws GWException {
        super(s3Parameter);
        logger = LoggerFactory.getLogger(DataPutBucketReplication.class);
    }
    ...
    public void extract() throws GWException {
        for (String headerName : Collections.list(s3Parameter.getRequest().getHeaderNames())) {
			...
			} else if (headerName.equalsIgnoreCase(GWConstants.X_AMZ_BUCKET_OBJECT_LOCK_TOKEN)) {
				bucketObjectLockToken = Strings.nullToEmpty(s3Parameter.getRequest().getHeader(headerName));
