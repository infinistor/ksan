...
public class DataListMultipartUploads extends S3DataRequest {
	...
	public DataListMultipartUploads(S3Parameter s3Parameter) throws GWException {
		super(s3Parameter);
		logger = LoggerFactory.getLogger(DataListMultipartUploads.class);
	}
	...
	public void extract() throws GWException {
		...
		maxUploads = s3Parameter.getRequest().getParameter(GWConstants.MAX_UPLOADS);
		if (Strings.isNullOrEmpty(maxUploads)) {
			logger.info(GWConstants.LOG_DATA_MAX_UPLOADS_NULL);
		}
		...
		uploadIdMarker = s3Parameter.getRequest().getParameter(GWConstants.UPLOAD_ID_MARKER);
		if (Strings.isNullOrEmpty(uploadIdMarker)) {
			logger.info(GWConstants.LOG_DATA_UPLOAD_ID_MARKER_NULL);
		}
