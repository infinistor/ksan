...
public class DataListBucketTagSearch extends S3DataRequest {
	...
	public DataListBucketTagSearch(S3Parameter s3Parameter) throws GWException {
		super(s3Parameter);
		logger = LoggerFactory.getLogger(DataListBucketTagSearch.class);
	}
	...
	public void extract() throws GWException {
		...
		marker = s3Parameter.getRequest().getParameter(GWConstants.MARKER);
		if (Strings.isNullOrEmpty(marker)) {
			logger.info(GWConstants.LOG_DATA_MARKER_NULL);
		}
        ...
        tag = s3Parameter.getRequest().getParameter(GWConstants.TAG);
        if (Strings.isNullOrEmpty(tag)) {
            logger.info(GWConstants.LOG_DATA_TAG_NULL);
        }
