...
public class DataPutBucketPolicy extends S3DataRequest {
    private String contentMD5;
	private String confirmRemoveSelfBucketAccess;
	private String expectedBucketOwner;
	private String policyJson;

    public DataPutBucketPolicy(S3Parameter s3Parameter) throws GWException {
        super(s3Parameter);
    ...
    public void extract() throws GWException {
        for (String headerName : Collections.list(s3Parameter.getRequest().getHeaderNames())) {
			...
			} else if (headerName.equalsIgnoreCase(GWConstants.X_AMZ_CONFIRM_REMOVE_SELF_BUCKET_ACCESS)) {
				confirmRemoveSelfBucketAccess = Strings.nullToEmpty(s3Parameter.getRequest().getHeader(headerName));
