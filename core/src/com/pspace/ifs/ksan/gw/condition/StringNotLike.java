...
public class StringNotLike extends PolicyCondition {
    public StringNotLike(JsonNode jsonNode) {
        super(jsonNode);
        logger = LoggerFactory.getLogger(StringNotLike.class);
    }
                ...
                if (tagging != null) {
                    if (tagging.tagset != null && tagging.tagset.tags != null) {
                        for (Tag t : tagging.tagset.tags) {
                            ...
                            if (!GWUtils.likematch(t.key, tagkey) && !GWUtils.likematch(t.value, tagvalue)) {
                                conditioncheck = true;
                                break;
                            }
                        }
