...
public class KsanStorageMove extends S3Request {

    public KsanStorageMove(S3Parameter s3Parameter) {
        super(s3Parameter);
        logger = LoggerFactory.getLogger(KsanStorageMove.class);
    }
    
    @Override
	public void process() throws GWException {
		logger.info(GWConstants.LOG_ADMIN_STORAGE_MOVE_START);
        ...
        } else if (s3Metadata.getTier().equalsIgnoreCase(GWConstants.AWS_TIER_GLACIER)) {
            s3Metadata.setTier(GWConstants.AWS_TIER_STANTARD);
        } else {
            logger.error("storage is not defined. : {}", s3Metadata.getTier());
        ...
        String diskpoolId = user.getUserDiskpoolId(s3Metadata.getTier());
        ...
        objectManager.storageMove(s3Parameter, objMeta, restoreObjMeta);
