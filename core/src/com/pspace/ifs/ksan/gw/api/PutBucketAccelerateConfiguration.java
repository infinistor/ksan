...
public class PutBucketAccelerateConfiguration extends S3Request {
    ...
    public void process() throws GWException {
        logger.info(GWConstants.LOG_PUT_BUCKET_ACCELERATE_START);
		...
		String accelerateXml = s3RequestData.getAccelerateXml();
        if (!Strings.isNullOrEmpty(accelerateXml)) {
			Accelerate accelerate = null;
			try {
				accelerate = new XmlMapper().readValue(accelerateXml, Accelerate.class);
			...
			if (accelerate == null) {
				logger.info("Accelerate class is null");
				throw new GWException(GWErrorCode.MALFORMED_X_M_L, s3Parameter);
			}

			if (accelerate.status != null && !accelerate.status.equalsIgnoreCase(GWConstants.STATUS_ENABLED) && !accelerate.status.equalsIgnoreCase(GWConstants.STATUS_SUSPENDED)) {
				throw new GWException(GWErrorCode.MALFORMED_X_M_L, s3Parameter);
			}

			if (!Strings.isNullOrEmpty(accelerate.status)) {
				updateBucketAccelerate(bucket, accelerate.status);
			}
		}
