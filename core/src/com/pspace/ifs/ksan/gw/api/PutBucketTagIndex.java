...
public class PutBucketTagIndex extends S3Request {
	...
	public void process() throws GWException {
		...
		if (!Strings.isNullOrEmpty(tagIndexXml)) {
			...
			if (tagIndex != null) {
				if (tagIndex.status != null) {
					if (tagIndex.status.equalsIgnoreCase(GWConstants.STATUS_ENABLED)) {
						updateBucketTagIndex(bucket, true);
					} else if (tagIndex.status.equalsIgnoreCase(GWConstants.STATUS_DISABLED)) {
