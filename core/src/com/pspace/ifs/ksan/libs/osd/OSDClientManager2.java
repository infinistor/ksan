...
import com.pspace.ifs.ksan.libs.osd.OSDClientPool2;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class OSDClientManager2 {
    private static Logger logger = LoggerFactory.getLogger(OSDClientManager2.class);
    private static Map<String, OSDClientPool2> pools = new HashMap<String, OSDClientPool2>();

    public static OSDClientManager2 getInstance() {
        return LazyHolder.INSTANCE;
    }

    private static class LazyHolder {
        private static OSDClientManager2 INSTANCE = new OSDClientManager2();
    }

    private OSDClientManager2() {
    }
        ...
        for (DiskPool diskpool : DiskManager.getInstance().getDiskPoolList()) {
            for (Server server : diskpool.getServerList()) {
                ...
                if (server.getStatus().equalsIgnoreCase(Server.STATUS_ONLINE) 
                    ...
                    OSDClientPool2 pool = new OSDClientPool2(server.getIp(), port, osdPerCount);
    ...
    public OSDClient getOSDClient(String ip) throws Exception {
        if (pools.containsKey(ip)) {
            OSDClientPool2 pool = pools.get(ip);
    ...
    public void releaseOSDClient(OSDClient client) throws Exception {
        if (pools.containsKey(client.getHost())) {
            OSDClientPool2 pool = pools.get(client.getHost());
