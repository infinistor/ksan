...
public class DataListParts extends S3DataRequest {
	...
	public DataListParts(S3Parameter s3Parameter) throws GWException {
		super(s3Parameter);
		logger = LoggerFactory.getLogger(DataListParts.class);
	}

	@Override
	public void extract() throws GWException {
		maxParts = s3Parameter.getRequest().getParameter(GWConstants.MAX_PARTS);
		if (Strings.isNullOrEmpty(maxParts)) {
			logger.info(GWConstants.LOG_DATA_MAX_PARTS_NULL);
		}
		
		partNumberMarker = s3Parameter.getRequest().getParameter(GWConstants.PART_NUMBER_MARKER);
		if (Strings.isNullOrEmpty(partNumberMarker)) {
			logger.info(GWConstants.LOG_DATA_PART_NUMBER_MARKER_NULL);
		}
		
		uploadId = s3Parameter.getRequest().getParameter(GWConstants.UPLOAD_ID);
		if (Strings.isNullOrEmpty(uploadId)) {
			logger.info(GWConstants.LOG_DATA_UPLOAD_ID_NULL);
		}
