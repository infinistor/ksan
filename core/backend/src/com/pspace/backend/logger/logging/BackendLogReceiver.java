...
public class BackendLogReceiver implements MQCallback {
	...
	public MQResponse call(String routingKey, String body) {
		...
		try {
			...
			// 변환 실패시
			if (event == null) {
				logger.error("Failed to convert to S3LogData");
				return new MQResponse(MQResponseType.ERROR, MQResponseCode.MQ_UNKNOWN_ERROR, "Failed to convert to S3LogData", 0);
			}

			// DB에 저장
			logger.info("BackendLogData : {}", event);
